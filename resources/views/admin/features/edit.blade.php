...
@section('content')
<div class="mb-6">
    <a href="{{ route('admin.features.index') }}" class="text-sky-600 hover:text-sky-800">← Back to Features</a>
    <h1 class="text-3xl font-bold text-gray-800 mt-2">Edit Feature</h1>
</div>
...
<div class="bg-white rounded-lg shadow p-6">
    <form action="{{ route('admin.features.update', $feature) }}" method="POST">
        ...
        <div class="mb-4">
            ...
            <input type="text" name="icon" id="icon" value="{{ old('icon', $feature->icon) }}" required
        ...
        <div class="mb-4">
            ...
            <textarea name="description" id="description" rows="3" required
                      class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-sky-500 focus:border-sky-500">{{ old('description', $feature->description) }}</textarea>
            @error('description')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="flex gap-4">
            ...
            <a href="{{ route('admin.features.index') }}" class="bg-gray-200 hover:bg-gray-300 text-gray-800 px-6 py-3 rounded-lg font-semibold transition">
                Cancel
            </a>
        </div>
    </form>
