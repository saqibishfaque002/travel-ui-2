...
<div class="mb-6">
    ...
    <h1 class="text-3xl font-bold text-gray-800 mt-2">Add New Feature</h1>
</div>
    ...
    <form action="{{ route('admin.features.store') }}" method="POST">
        ...
        <div class="mb-4">
            ...
            <textarea name="description" id="description" rows="3" required
                      class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-sky-500 focus:border-sky-500"
                      placeholder="Your transactions are protected with enterprise-level encryption.">{{ old('description') }}</textarea>
            @error('description')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            @enderror
        </div>
