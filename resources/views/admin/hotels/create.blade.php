...
<div class="mb-6">
    ...
    <h1 class="text-3xl font-bold text-gray-800 mt-2">Add New Hotel</h1>
</div>
    ...
    <form action="{{ route('admin.hotels.store') }}" method="POST">
        ...
        <div class="grid grid-cols-3 gap-4 mb-4">
            <div>
                ...
                <input type="number" step="0.1" min="0" max="5" name="rating" id="rating" value="{{ old('rating') }}" required
                       class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-sky-500 focus:border-sky-500"
                       placeholder="4.8">
                @error('rating')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            ...
            <div>
                ...
                <input type="number" min="0" name="reviews" id="reviews" value="{{ old('reviews') }}" required
                       class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-sky-500 focus:border-sky-500"
                       placeholder="1250">
                @error('reviews')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
        ...
        <div class="mb-4">
            ...
            <input type="text" name="image" id="image" value="{{ old('image') }}" required
                   class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-sky-500 focus:border-sky-500"
                   placeholder="luxury-hotel-exterior.jpg">
